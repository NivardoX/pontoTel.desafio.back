...
def post_roles():
    ...
    try:
        db.session.commit()
        return jsonify({'message': Messages.REGISTER_SUCCESS_CREATED.format("Roles"), 'has_error': False})
...
def patch_roles(id):
    ...
    if not roles:
        return jsonify({'message': Messages.REGISTER_NOT_FOUND.format(id), 'has_error': True})
    ...
    try:
        db.session.commit()
        return jsonify({'message': Messages.REGISTER_SUCCESS_UPDATED.format("Roles"), 'has_error': False})
    except exc.IntegrityError:
        db.session.rollback()
        return jsonify({'message': Messages.REGISTER_CHANGE_INTEGRITY_ERROR, 'has_error': True})
...
def delete(self,id):
    ...
    try:
        db.session.commit()
        return jsonify({'message': Messages.REGISTER_SUCCESS_DELETED.format("Roles"), 'has_error': False})
    except exc.IntegrityError:
        db.session.rollback()
        return jsonify({'message': Messages.REGISTER_CHANGE_INTEGRITY_ERROR, 'has_error': True})
